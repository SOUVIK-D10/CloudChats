...
public class ThreadService {
    ...
    public Long createPrivateThread(int userId, String username) throws GeneralException {
        ...
        Thread thread = new Thread(participantsId, "Private Chat", ThreadConstant.THREAD_TYPE_PRIVATE, userId);
        thread = threadRepo.save(thread);
        return thread.getId();
    }
    public List<ThreadListItemDTO> getAllThreads(int userId) {
        List<Thread> list = threadRepo.findAllForMy(userId);
    ...
    public OpenedThreadDTO createGroupThread(
        ...
        Thread group = new Thread(participantIds, name, ThreadConstant.THREAD_TYPE_GROUP, userId);
